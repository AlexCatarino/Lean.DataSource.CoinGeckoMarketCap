...
{
    /// <summary>
    /// CoinGeckoMarketCapUniverseDataDownloader implementation.
    /// </summary>
    public class CoinGeckoMarketCapUniverseDataDownloader : IDisposable
    {
        ...
        public CoinGeckoMarketCapUniverseDataDownloader(string destinationFolder, string apiKey = null)
        {
        ...
        public bool Run()
        {
            ...
            Log.Trace($"CoinGeckoMarketCapUniverseDataDownloader.Run(): Finished in {stopwatch.Elapsed.ToStringInvariant(null)}");
            return true;
        }
            ...
            for (var retries = 1; retries <= _maxRetries; retries++)
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        {
                            Log.Error($"CoinGeckoMarketCapUniverseDataDownloader.HttpRequester(): Files not found at url: {Uri.EscapeUriString(url)}");
                ...
                {
                    Log.Error(e, $"CoinGeckoMarketCapUniverseDataDownloader.HttpRequester(): Error at HttpRequester. (retry {retries}/{_maxRetries})");
                    Thread.Sleep(1000);
                }
            }
